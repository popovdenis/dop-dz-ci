...
class Paypal_Lib
{
    ...
    function image($file)
    {
        $this->submit_btn = '<input type="image" name="add" src="' . base_url(
            ) . $this->button_path . '/' . $file . '" border="0" />';
    }
    ...
    function validate_ipn()
    {
        ...
        $post_string = '';
        if (isset($_POST)) {
            foreach ($_POST as $field => $value) {       // str_replace("\n", "\r\n", $value)
        ...
        if (substr_count($this->ipn_response, "VERIFIED")) {
